# R script to directly convert .Rd documentation to Sphinx Restructured Text .rst files.
# Should be run from source/ subdirectory.

source("./scripts/Rd2RstFunctions.R") # to load helper functions.

### Runtime parameters / file-locations to specify: ###
rd_dir = './api/man/' # directory of R documentation files.
rstfiles_dir = './api/' # Directory where to store index.rst and other Sphinx toctrees.
...
rstheader = '' # header for each RST function file.
...
# file.remove(dir(rst_dir, pattern = "*.rst", full.names = TRUE))
...
for (i in 1:length(func_names)) {
    ...
    if (length(title_line) == 1) {
        ...
        if ("Example::" %in% title_string || "Examples::" %in% title_string) {
            example_in_desc_index = grep("Example::|Examples::", title_string)[[1]]
            example_in_desc = title_string[(example_in_desc_index+1):length(title_string)]
            title_string = title_string[0:(example_in_desc_index-1)]
        }
    ...
    func_desc[i] = gsub("\n"," ",trimws(short_desc))
    func_desc[i] = sub("\\.\\s.*", "", func_desc[i])
    func_desc[i] = sub("\\.$", "", func_desc[i])
    ...
    if (length(example_in_desc) > 1) {
        example_in_desc = paste0("\t",trimws(example_in_desc))
        formatted_description = c(formatted_description, "","**Example**::","", example_in_desc)
    }
    ...
    rstcontents = c(rstcontents, rstfooter)
    writeLines(rstcontents, rstfile)
}


### Create RST toctree files ###

# First categorize functions to form toctree groupings:
...
writeLines(c(index_rstheader, index_rstformatted, rstfooter), paste0(rstfiles_dir,"index.rst"))
...
writeLines(c(ndarray_rstheader, ndarray_attribute_header, formatFuncsToctree(nd_attribute_funcs,func_names,func_desc),
    ...
    rstfooter), paste0(rst_dir,"ndarray.rst"))
...
writeLines(c(symbol_rstheader, symbol_attribute_header, formatFuncsToctree(symbol_attribute_funcs,func_names,func_desc),
    ...
    symbol_neuralnet_header, formatFuncsToctree(symbol_neuralnet_funcs,func_names,func_desc),
            rstfooter), paste0(rst_dir,"symbol.rst"))
...
writeLines(c(io_rstheader, formatFuncsToctree(io_funcs,func_names,func_desc),
            ...
            rstfooter), paste0(rst_dir,"io.rst"))
...
writeLines(c(context_rstheader, formatFuncsToctree(context_funcs,func_names,func_desc),
             rstfooter), paste0(rst_dir,"context.rst"))
...
writeLines(c(model_rstheader, formatFuncsToctree(model_funcs,func_names,func_desc),
             rstfooter), paste0(rst_dir,"model.rst"))
...
writeLines(c(metric_rstheader, formatFuncsToctree(metric_funcs,func_names,func_desc),
             rstfooter), paste0(rst_dir,"metric.rst"))
...
writeLines(c(optimization_rstheader,optimization_rst_opt, formatFuncsToctree(optimization_opt_funcs,func_names,func_desc),
            ...
            rstfooter), paste0(rst_dir,"optimization.rst"))
...
writeLines(c(random_rstheader, random_rst_rest, formatFuncsToctree(random_rest_funcs,func_names,func_desc),
            ...
            rstfooter), paste0(rst_dir,"random.rst"))
...
writeLines(c(advanced_rstheader, formatFuncsToctree(kv_funcs,func_names,func_desc), 
             ...
             rstfooter), paste0(rst_dir,"advanced.rst"))
...
writeLines(c(monitoring_rstheader, formatFuncsToctree(callback_funcs,func_names,func_desc), 
             ...
             rstfooter), paste0(rst_dir,"monitoring.rst"))
...
writeLines(c(allfunctions_rstheader, paste0("   ",func_names),
             rstfooter), paste0(rst_dir,"allfunctions.rst"))
